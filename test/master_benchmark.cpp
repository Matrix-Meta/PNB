...
void test_bit_linear_xmx(s::queue &q)
{
            ...
            for (size_t b = 0; b < batch && passed; b++) {
                for (size_t n = 0; n < out_features && passed; n++) {
                    ...
                    if (std::abs(actual - expected) > 1e-3f) {
                        passed = false;
                        std::cout << "    Mismatch at [" << b << "," << n << "]: expected " 
                                  << expected << ", got " << actual << "\n";
                    }
                }
            }
    ...
    // Performance smoke test (small scale)
    {
        ...
        std::cout << "  Performance smoke test: " << std::fixed << std::setprecision(2) 
                  << gflops << " GFLOPS\n";
...
void test_slm_tiling_comparison(s::queue &q)
{
    ...
    for (const auto& config : configs) {
        ...
        // Basic XMX
        {
            ...
            layer.set_weights(h_W);
            for(int i=0; i<3; ++i) layer.forward(buf_X, buf_Y_basic, config.batch);
            q.wait();
            
            auto start = std::chrono::high_resolution_clock::now();
            for (int i = 0; i < test_iters; i++) layer.forward(buf_X, buf_Y_basic, config.batch);
            q.wait();
            auto end = std::chrono::high_resolution_clock::now();
            time_basic = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() / (test_iters * 1000.0);
        }
        ...
        // XMX + SLM Tiling
        {
            ...
            layer.set_weights(h_W);
            for(int i=0; i<3; ++i) layer.forward(buf_X, buf_Y_slm, config.batch);
            q.wait();
            
            auto start = std::chrono::high_resolution_clock::now();
            for (int i = 0; i < test_iters; i++) layer.forward(buf_X, buf_Y_slm, config.batch);
            q.wait();
            auto end = std::chrono::high_resolution_clock::now();
            time_slm = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() / (test_iters * 1000.0);
        }
...
void test_spike_neuron(s::queue &q)
{
...

    {
        ...
        report_test({"SpikeNeuron forward_single", passed, 
                     "No spikes generated (got " + std::to_string(total_spikes) + ")"});
    }
}
...
void test_bit_brain(s::queue &q)
{
    ...
    config.output_dim = 8;
    config.batch_size = 1;
    config.min_think_steps = 1;
    config.max_think_steps = 5;
...
int main()
{
    try
    {
        ...
        for (auto [b, m, n] : std::vector<std::tuple<size_t, size_t, size_t>>{
            {1, 256, 256}, {1, 1024, 1024}, {1, 4096, 4096},
            {16, 1024, 1024}, {64, 1024, 1024}, {256, 1024, 1024},
            {512, 2048, 2048}, {1024, 2048, 2048}}) 
        {
            ...
            for(int i=0; i<3; ++i) layer.forward(buf_X, buf_Y, b);
            q.wait();
            
            auto start = std::chrono::high_resolution_clock::now();
            for(int i=0; i<10; ++i) layer.forward(buf_X, buf_Y, b);
            q.wait();
            auto end = std::chrono::high_resolution_clock::now();
            ...
            if (mode_enum == neurobit::layers::BitLinearXMX<bf16>::Mode::Standard) mode_str = "STD";
            else if (mode_enum == neurobit::layers::BitLinearXMX<bf16>::Mode::XMX_Universal) mode_str = "XMX_UNI";
            else if (mode_enum == neurobit::layers::BitLinearXMX<bf16>::Mode::XMX_SLM_Medium) mode_str = "XMX_SLM_Medium";
            else if (mode_enum == neurobit::layers::BitLinearXMX<bf16>::Mode::XMX_SLM_Large) mode_str = "XMX_SLM_Large";
            
            char config[32];
            snprintf(config, sizeof(config), "%zux%zux%zu", b, m, n);
            printf("| %-19s | %10zu | %9.3f | %10.3f | %8.2f | %-8s |\n",
                   config, b*m*n, time_ms, tops, gb_s, mode_str.c_str());
        }
        std::cout << "+---------------------+------------+-----------+------------+----------+----------+\n";
            ...
            {1, 1, 4096}, {1, 1, 65536}, {64, 1, 4096}, {256, 1, 4096}}) 
        {
            ...
            for(int i=0; i<3; ++i) layer.forward(q, buf_X, buf_A, buf_Y, buf_State);
            q.wait();
            
            auto start = std::chrono::high_resolution_clock::now();
            const int iters = 100;
            for(int i=0; i<iters; ++i) layer.forward(q, buf_X, buf_A, buf_Y, buf_State);
            q.wait();
            auto end = std::chrono::high_resolution_clock::now();
        ...
        for (auto [b, n] : std::vector<std::pair<size_t, size_t>>{
            {1, 4096}, {1, 65536}, {64, 16384}}) 
        {
            neurobit::layers::SpikeNeuron::Config cfg{b, 1, n, 1.0f, 0.5f, true};
            neurobit::layers::SpikeNeuron layer(cfg);
            ...
            auto start = std::chrono::high_resolution_clock::now();
            const int iters = 100;
            for(int i=0; i<iters; ++i) layer.forward_single(q, buf_X, buf_V, buf_Z, buf_OU, buf_Act);
            q.wait();
            auto end = std::chrono::high_resolution_clock::now();
