...
namespace neurobit
{
    namespace core
    {
        ...
        public:
             ...
             */
            static constexpr size_t get_tile_m() { return 16; }
            static constexpr size_t get_tile_n() { return 16; }
            static constexpr size_t get_tile_k() { return 16; }
            ...
            static void print_support_info(const s::device& device) {
                ...
                if (supported) {
                    std::cout << "  Optimal tile: " << get_tile_m() << "×" << get_tile_k() 
                              << " × " << get_tile_k() << "×" << get_tile_n() << "\n";
        ...
        public:
            explicit AdaptiveMatMul(s::queue& q) 
                : queue_(q),
                  use_xmx_(XMXSupport::is_supported(q.get_device()))
            {
            }
        ...
        private:
            ...
            void multiply_standard(
                ...
                queue_.submit([&](s::handler& h) {
                    ...
                    h.parallel_for(
                        s::nd_range<2>{{global_m, global_n}, {TILE_SIZE, TILE_SIZE}},
                        [=](s::nd_item<2> it) {
