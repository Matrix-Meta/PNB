    ...
    namespace core
    {
        ...
        public:
            ...
            size_t compute_optimal_batch(
                ...
                // 目標 2: 記憶體限制
                // 考慮輸入、輸出、權重、狀態
                size_t memory_per_sample = 
                    (input_dim + hidden_dim * 2) * precision_bytes;  // I/O + state
                
                size_t weight_memory = hidden_dim * input_dim / 4;  // Bit-packed
                
                size_t available_memory = static_cast<size_t>(max_memory_ * 0.7);  // 70% 安全邊界
                size_t max_batch_by_memory = 
                    (available_memory - weight_memory) / memory_per_sample;
                ...
                size_t work_groups_needed = 
                    (elements_per_batch + max_work_group_size_ - 1) / max_work_group_size_;
                size_t optimal_for_work_groups = 
                    std::max(xe_cores_ / work_groups_needed, size_t(1)) * 16;
            ...
            size_t compute_tile_size(size_t matrix_dim) const {
                ...
                size_t max_tile_from_memory = 
                    static_cast<size_t>(std::sqrt(local_memory_ / 4));  // 保守估計
            ...
            bool should_use_xmx(
                ...
                size_t tile_m = 8, size_t tile_k = 16, size_t tile_n = 16
            ) const {
                // XMX 對齊要求
                bool is_aligned = 
                    (M % tile_m == 0) && 
             ...
             * 打印調度信息
             */
            void print_schedule_info(
                size_t input_dim,
                size_t hidden_dim,
                size_t precision_bytes = 2
            ) const {
             ...
             * 獲取設備信息
             */
            size_t get_xe_cores() const { return xe_cores_; }
            size_t get_max_memory() const { return max_memory_; }
            size_t get_local_memory() const { return local_memory_; }
        };
