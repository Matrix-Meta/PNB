    ...
    namespace core
    {
        ...
        public:
            ...
            static bool is_battlemage(const s::device& device) {
                if (!is_intel_gpu(device)) return false;
                
                auto name = device.get_info<s::info::device::name>();
                // B580, B770 等都是 Battlemage
                return name.find("Arc(TM) B") != std::string::npos ||
         ...
         */
        template<typename PrecisionPolicy = struct AutoSelect>
        ...
        public:
            ...
            using compute_type = std::conditional_t<
                std::is_same_v<PrecisionPolicy, struct ForceBF16>,
                bfloat16,
                std::conditional_t<
                    std::is_same_v<PrecisionPolicy, struct ForceFP16>,
                    s::half,
        ...
        public:
            explicit MixedPrecision(s::queue& q) 
                : queue_(q), 
                  precision_(PrecisionSelector::detect_best_precision(q)) 
            {}
            ...
            void convert_to_compute(
                s::buffer<storage_type, 1>& src,
                s::buffer<T, 1>& dst
            ) {
                size_t N = src.size();
                queue_.submit([&](s::handler& h) {
                    ...
                    h.parallel_for(s::range<1>(N), [=](s::id<1> i) {
             ...
             */
            template<typename T = compute_type>
            void convert_from_compute(
                s::buffer<T, 1>& src,
                s::buffer<storage_type, 1>& dst
            ) {
                size_t N = src.size();
                queue_.submit([&](s::handler& h) {
                    auto in = src.template get_access<s::access::mode::read>(h);
                    auto out = dst.template get_access<s::access::mode::write>(h);
                    h.parallel_for(s::range<1>(N), [=](s::id<1> i) {
                        out[i] = static_cast<storage_type>(in[i]);
                    });
                });
            }
        ...
        // 策略標籤
        struct AutoSelect {};   // 自動選擇 (BF16 > FP16 > FP32)
        struct ForceBF16 {};    // 強制 BF16
        struct ForceFP16 {};    // 強制 FP16
        struct ForceFP32 {};    // 強制 FP32
        ...
        public:
            explicit RuntimePrecisionSelector(const s::queue& q) 
                : precision_(PrecisionSelector::detect_best_precision(q)) 
            {}
            ...
            void execute(s::queue& q, Kernel&& kernel) {
                switch (precision_) {
