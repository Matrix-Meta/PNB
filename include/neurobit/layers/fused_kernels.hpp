...
namespace neurobit
{
    namespace layers
    {
        ...
        public:
            explicit FusedForward(s::queue& q, size_t hidden_dim)
                : queue_(q), hidden_dim_(hidden_dim)
            {}
            ...
            void forward(
                s::buffer<T>& X,
                s::buffer<uint8_t>& W_packed,
                s::buffer<T>& state,
                s::buffer<T>& memory,
                s::buffer<T>& Y,
                float A, float B, float familiarity,
                size_t batch_size,
                size_t input_dim,
                size_t output_dim
            ) {
            ...
            void forward_vectorized(
                ...
                queue_.submit([&](s::handler& h) {
                    ...
                    h.parallel_for(
                        s::nd_range<2>{{batch_size, output_dim / VEC_SIZE}, {1, 64}},
                        [=](s::nd_item<2> it) {
                            ...
                            for (size_t v = 0; v < VEC_SIZE; ++v) {
                                ...
                                // Memory injection
                                if (familiarity > 0.01f) {
                                    state_vec[v] = state_vec[v] * (1.0f - familiarity * 0.5f) + 
                                                  mem_vec[v] * familiarity;
                                }
        ...
        public:
            FusedSNNInference(s::queue& q, size_t num_steps)
                : queue_(q), num_steps_(num_steps)
            {}
