...
namespace neurobit
{
    namespace layers
    {
        ...
        template<typename ComputeT = bfloat16>
        class BitLinearXMX
        {
        private:
            s::queue& queue_;
        ...
        public:
            ...
            void set_weights(const std::vector<int8_t>& weights) {
                if (weights.size() != input_dim_ * output_dim_) throw std::invalid_argument("Size mismatch");
            ...
            enum class Mode { Standard, XMX_Universal, XMX_SLM_Medium, XMX_SLM_Large };
            Mode get_mode(size_t batch_size) const {
                if (!use_xmx_) return Mode::Standard;
                if (!use_slm_tiling_) return Mode::XMX_Universal;
                if (batch_size >= 256 && output_dim_ >= 128 && input_dim_ >= 32) return Mode::XMX_SLM_Large;
                if (batch_size >= 64 && output_dim_ >= 64 && input_dim_ >= 32) return Mode::XMX_SLM_Medium;
                return Mode::XMX_Universal;
            }

        private:
            void update_packed_weights(s::buffer<int8_t, 1>& W_ext) {
                size_t K = input_dim_; size_t N = output_dim_;
                queue_.submit([&](s::handler& h) {
            ...
            // ... forward_xmx_universal ... (Keeping it as is for robustness)
            template <typename InputT, typename OutputT>
            void forward_xmx_universal(s::buffer<InputT>& X, s::buffer<OutputT>& Y, size_t M) {
                const size_t K = input_dim_; const size_t N = output_dim_;
                size_t num_m = (M + TM - 1) / TM; size_t num_n = (N + TN - 1) / TN;
                queue_.submit([&](s::handler& h) {
                    ...
                    h.parallel_for(s::nd_range<2>{s::range<2>{num_m, num_n*16}, s::range<2>{1, 16}}, [=](s::nd_item<2> it) [[sycl::reqd_sub_group_size(16)]] {
                        ...
                        for(size_t k=0; k<K; k+=TK) {
                            for(int r=0; r<TM; ++r) {
                                size_t cur_m = m_base + r; size_t cur_k = k + lane;
                                slm_x[r][lane] = (cur_m < M && cur_k < K) ? ComputeT(acc_x[cur_m*K+cur_k]) : ComputeT(0);
                            }
                            if(k+lane < K) {
                                for(int c_blk=0; c_blk<TN/4; ++c_blk) {
                                    size_t c_start = c_blk*4;
                                    for(int b=0; b<4; ++b) {
                                        size_t c = c_start+b;
                                        if(n_base+c < N) {
                                            size_t widx = (k+lane)*N + (n_base+c);
                                            uint8_t bits = (acc_w[widx/4] >> ((widx%4)*2)) & 0x03;
                                            slm_w[lane][c] = ComputeT(static_cast<float>(bits) - 1.0f);
                                        } else slm_w[lane][c] = ComputeT(0);
                                    }
                                }
                            } else {
                                for(int c=0; c<TN; ++c) slm_w[lane][c] = ComputeT(0);
                            }
                            s::group_barrier(sg);
                            matrix::joint_matrix<s::sub_group, ComputeT, matrix::use::a, TM, TK, matrix::layout::row_major> ma;
                            matrix::joint_matrix<s::sub_group, ComputeT, matrix::use::b, TK, TN, matrix::layout::row_major> mb;
                            matrix::joint_matrix_load(sg, ma, slm_x.template get_multi_ptr<s::access::decorated::no>(), TK);
                            matrix::joint_matrix_load(sg, mb, slm_w.template get_multi_ptr<s::access::decorated::no>(), TN);
                            matrix::joint_matrix_mad(sg, acc, ma, mb, acc);
                            s::group_barrier(sg);
                        }
                        matrix::joint_matrix_store(sg, acc, slm_acc.template get_multi_ptr<s::access::decorated::no>(), TN, matrix::layout::row_major);
                        s::group_barrier(sg);
                        for(int r=0; r<TM; ++r) {
                            if(m_base+r < M && n_base+lane < N) acc_y[(m_base+r)*N + n_base+lane] = OutputT(slm_acc[r][lane]);
                        }
                    });
                });
            }
            ...
            void forward_xmx_slm_template(s::buffer<InputT>& X, s::buffer<OutputT>& Y, size_t M) {
                ...
                queue_.submit([&](s::handler& h) {
                    auto acc_x = X.template get_access<s::access::mode::read>(h);
                    auto acc_y = Y.template get_access<s::access::mode::write>(h);
                    // Reinterpret W as uint32_t for vectorized load?
                    // Safe in SYCL to cast accessor pointer in kernel? Yes, usually.
                    // But accessing W_packed_ (uint8) as uint32 requires alignment.
                    // BitPackedWeights are typically aligned enough.
                    auto acc_w = W_packed_.template get_access<s::access::mode::read>(h);
                    ...
                    h.parallel_for(
                        ...
                        [=](s::nd_item<2> it) [[sycl::reqd_sub_group_size(16)]] {
                            ...
                            // 1. Vectorized Load X: float4 (128-bit)
                            // TILE_M * TILE_K elems (e.g. 64x32=2048). 2048*4 bytes = 8KB.
                            // 256 threads. 32 bytes/thread = 8 floats/thread = 2 x float4.
                            constexpr int ELEMS_X = TILE_M * TILE_K;
                            constexpr int FLOATS_PER_THREAD = (ELEMS_X + THREADS - 1) / THREADS;
                            constexpr int VEC4_LOOPS = FLOATS_PER_THREAD / 4;
                            constexpr int REMAIN_LOOPS = FLOATS_PER_THREAD % 4;

                            // 2. Vectorized Load W: uint32_t (32-bit) = 16 weights
                            // TILE_K * TILE_N weights (e.g. 32x128=4096). 4096/4 = 1024 bytes.
                            // 256 threads. 4 bytes/thread = 1 x uint32.
                            constexpr int WEIGHTS_PER_TILE = TILE_K * TILE_N;
                            constexpr int BYTES_PER_TILE = WEIGHTS_PER_TILE / 4;
                            constexpr int INTS_PER_THREAD = (BYTES_PER_TILE + 3) / 4 / THREADS; // May be 0 if tile small?
                            // For Medium (32x64): 2048w = 512B. 256T -> 2B/thread. No full int.
                            // For Large (64x128): 8192w = 2048B. 256T -> 8B/thread = 2 ints.
                            // Let's implement generic u32 load loop.

                            // Pointer casting for W
                            // Use multi_ptr to get raw pointer safely?
                            // Or just byte addressing. 4 bytes = 1 int.
                            
                            for(size_t k_base=0; k_base < K; k_base += TILE_K) {
                                // 1. Load X (Vectorized float4)
                                // Only if InputT is float. If bf16, need bf16 vec.
                                // Assuming InputT=float for now as per master_benchmark.
                                ...
                                // 2. Load W (Vectorized uint32)
                                // We iterate over BYTES.
                                // We want to load 4 bytes at once if possible.
                                // Let's just do simple byte load loop but unrolled, 
                                // because casting to uint32* might violate alignment if K*N is not aligned.
                                // BUT, we can optimize the index calc.
                                
                                // Total bytes to load:
                                const int bytes_to_load = (TILE_K * TILE_N) / 4;
                                const int bytes_per_thread = (bytes_to_load + THREADS - 1) / THREADS;
                                
                                #pragma unroll
                                for(int i=0; i < bytes_per_thread; ++i) {
                                    int byte_idx = loc_id * bytes_per_thread + i;
                                    if (byte_idx < bytes_to_load) {
                                        // Calculate Global Coord from Byte Index
                                        // Tile is a block. Global memory is linear row-major.
                                        // We need to map byte_idx (0..1023) to (r, c_byte) in tile.
                                        ...
                                        // Global coord
                                        int gx = k_base + r;
                                        int gy_byte = (n_base / 4) + c_byte; // byte column index
                                        
                                        // One byte contains weights for cols: gy_byte*4 ... gy_byte*4+3
                                        // Global packed index: gx * (N/4) + gy_byte
                                        // Warning: This assumes N is multiple of 4.
                                ...
                                for(int k=0; k<TILE_K; k+=TK) {
                                    matrix::joint_matrix<s::sub_group, ComputeT, matrix::use::a, TM, TK, matrix::layout::row_major> mat_x;
                                    matrix::joint_matrix<s::sub_group, ComputeT, matrix::use::b, TK, TN, matrix::layout::row_major> mat_w;
                                    
                                    matrix::joint_matrix_load(sg, mat_x, slm_x.template get_multi_ptr<s::access::decorated::no>() + sg_m_off*TILE_K + k, TILE_K);
                                    matrix::joint_matrix_load(sg, mat_w, slm_w.template get_multi_ptr<s::access::decorated::no>() + k*TILE_N + sg_n_off, TILE_N);
                            ...
                            matrix::joint_matrix_store(sg, acc, slm_ptr + sg_m_off*TILE_N + sg_n_off, TILE_N, matrix::layout::row_major);
                            s::group_barrier(sg);

                            const size_t items_y = TILE_M * TILE_N;
                            #pragma unroll
                            for(int i=0; i<(items_y + THREADS - 1)/THREADS; ++i) {
                                size_t lin = loc_id + i * THREADS;
                                if (lin < items_y) {
                                    size_t r = lin / TILE_N;
                                    size_t c = lin % TILE_N;
