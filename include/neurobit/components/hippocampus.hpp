...
namespace neurobit
{
    namespace components
    {
        ...
        public:
            ...
            Hippocampus(const Config &cfg)
                : cfg_(cfg),
                  last_familiarity_(0.0f),
                  learned_threshold_(1.0f)
            {
            }
             ...
             * 使用餘弦相似度衡量輸入與記憶的匹配程度
             */
            RecallResult compute_familiarity(
                s::queue &q,
                s::buffer<float, 1> &buf_X,       // 輸入 [input_dim]
            ...
            void learn(
                ...
                s::buffer<float, 1> &buf_Z,       // SNN 輸出 (spike)
             ...
             * 獲取學習到的閾值
             */
            float get_learned_threshold() const { return learned_threshold_; }

            /**
             * 設置學習到的閾值
             */
            void set_learned_threshold(float th) { learned_threshold_ = th; }

            /**
             * 獲取上次熟悉度
             */
            float get_last_familiarity() const { return last_familiarity_; }
